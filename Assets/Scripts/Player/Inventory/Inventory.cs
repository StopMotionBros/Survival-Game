...
public class Inventory : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Input.GetMouseButtonUp(0))
		{
			...
			if (!_grid.InsideGrid(x, y) && !_grid.InsideGrid(x + w, y + h))
			{
				...
				Inventory hoveredContainer = null;
				foreach (RaycastResult ray in results)
				{
					if (!ray.gameObject.CompareTag("Inventory")) continue;

					hoveredContainer = ray.gameObject.GetComponent<Inventory>();
	...
	public bool RemoveItem(ItemData item, int amount) => RemoveItem(item, amount, out _);
	public bool RemoveItem(ItemData item, int amount, out int remainder)
	{
		remainder = amount;
		...
		UIItemSlot slot = FindSlotWithItem(item);
		if (!slot) return false;

		slot.RemoveItem(amount, out remainder);
		if (remainder > 0) RemoveItem(item, remainder, out remainder);
	...
	public bool HasItem(ItemData item, int amount = 1)
	{
		...
		UIItemSlot[] slots = FindSlotsWithItem(item);
		if (slots.Length == 0) return false;

		int slotAmount = 0;
		foreach (UIItemSlot slot in slots)
		{
			slotAmount += slot.Slot.Amount;
		}
	...
	public UIItemSlot[] FindSlotsWithItem(ItemData item)
	{
		List<UIItemSlot> slots = new();
		foreach (UIItemSlot slot in _slots)
		{
			if (slot.Slot.Item == item)
				if(!slot.Slot.Empty)
